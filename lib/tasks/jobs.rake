...
namespace :jobs do
  ...
  task update: :environment do
    def scrape(json, source)
      file_data = File.read("db/scrapes/#{json}")
    ...
    scrape("cfn.json", "CFN")
    scrape("wantedly.json", "Wantedly")
    scrape("japandev.json", "Japandev")
    ...
    #   file_data = File.read('db/scrapes/2026-09-04wantedly.json')
    #   data_array = JSON.parse(file_data)
    #   data_array.each do |data|
    #     company = Company.find_or_create_by(name: data["company_name"])
    #     company.update(
